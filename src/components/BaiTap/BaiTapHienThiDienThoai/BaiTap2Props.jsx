...
const BaiTap2Props = () => {
  const { phones } = useSelector((state) => state.dienThoaiSlice);
  let [dienThoai, setChiTietDienThoai] = useState(phones[0]);
  function updateDienThoai(dienThoai) {
    setChiTietDienThoai(dienThoai);
  }
  return (
    <div>
      <h2>Bai Tap Hien thi dien thoai</h2>
      <DanhSachDienThoai
        updateDienThoai={updateDienThoai}
        listPhone={phones}
        title={"Danh Sach dien thoai"}
      />
